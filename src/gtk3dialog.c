/*
 * gtk3dialog.c: Starting up the program.
...
get_program_from_variable(gchar *name)
{
	...
	if (program_src == NULL) 
		g_error(
"Gtk3dialog: Could not find the dialog description in the environment "
"variable '%s'.", name);
...
void
load_styles_file(gchar *filename)
{
...
main(int argc, char *argv[])
{
	...
	if (option_input_stdin) {
		...
		goto gtk3dialog_initialized;
	}
	/*
	 * If we have an event driven program we recall the gtk3dialog with
